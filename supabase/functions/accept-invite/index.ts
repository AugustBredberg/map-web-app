...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceRoleKey) {
      console.error("[accept-invite] Missing env vars", { supabaseUrl: !!supabaseUrl, serviceRoleKey: !!serviceRoleKey });
    ...
    // Create the user with email_confirm: true — the invite link is proof of ownership
    ...
    if (createError) {
      ...
      if (msg.includes("already registered") || msg.includes("already exists")) {
        return new Response(JSON.stringify({ error: createError.message, code: "account_exists" }), {
          status: 409,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    // Add to organization (ignore if somehow already a member)
    const { error: memberError } = await admin
      .from("organization_members")
      .insert({ organization_id: inv.organization_id, user_id: userId, role: "member", display_name: name });
    ...
    // Delete the invitation — token is now consumed
